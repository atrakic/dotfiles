...
function install_vault() {
  ...
  local file=$DIR/$bin
  if [ ! -x "$file" ] || [ "$force" ]; then
    url=$(curl --silent https://releases.hashicorp.com/index.json | jq '{vault}' |egrep "linux_amd64" |grep url | egrep -v "beta|alpha"| sort -rV | head -1 | awk -F'/' '{print $5}')
    cd /tmp || exit 1
    rm -rf $bin
...
function install_terraform() {
  ...
  local file=$DIR/$bin
  if [ ! -x "$file" ] || [ "$force" ]; then
    url=$(curl --silent https://releases.hashicorp.com/index.json | jq '{terraform}' |egrep "linux_amd64" |grep url | egrep -v "beta|alpha" | sort -rV | head -1 | awk -F'/' '{print $5}')
    cd /tmp/ || exit 1
    rm -rf $bin
    curl -o $bin.zip -sL "${url}"
    unzip -o $bin.zip
...
function install_packer() {
  ...
  local file=$DIR/$bin
  if [ ! -x "$file" ] || [ "$force" ]; then
    url=$(curl --silent https://releases.hashicorp.com/index.json | jq '{packer}' |egrep "linux_amd64" |grep url | egrep -v "beta|alpha" | sort -rV | head -1 | awk -F'/' '{print $5}')
    cd /tmp/ || exit 1
    curl -o $bin.zip -sL "${url}"
    unzip -o $bin.zip
    mv -f "$bin" "$DIR"
